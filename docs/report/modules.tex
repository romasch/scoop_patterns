...
% This section only describes the descendants of \lstinline!CP_IMPORTER! which support writing the feature manually and provide predefined import routines for some types.
...
% Because writing an extra class for every importable object may be tedious, there's a support class \lstinline!CP_IMPORTABLE!:
...
There are two classes which can be used for \lstinline!CP_IMPORTABLE! objects: 
\begin{itemize}
...
The feature \lstinline!start! is used to start the loop.
...
It also defines the interface to cancel an operation or to get the progress percentage (e.g. for a download task), but these queries need to be supported by the operation as well.
...
This means that both the \lstinline!CP_SHARED_RESULT_PROMISE! and the \lstinline!CP_RESULT_PROMISE_PROXY! have an additional generic argument which defines the import strategy.
...
Therefore we added a new, deferred class \lstinline!CP_TASK!.
The class represents an importable asynchronous operation and adds some additional functionality, like exception handling or the ability to add a promise object (see Section \ref{sec:promise}).
To define a new task a user needs to inherit from \lstinline!CP_DEFAULT_TASK! and implement the two features \lstinline!run! and \lstinline!make_from_separate!.

The second important interface is provided by the class \lstinline!CP_EXECUTOR!.
It introduces the feature \lstinline!put! to execute a separate \lstinline!CP_TASK! object.
% The executor framework therefore basically consists of the two deferred classes \lstinline!CP_TASK! and \lstinline!CP_EXECUTOR!.
...
The executor framework is pretty useless on its own, as it basically only consists of two deferred classes.
...
That way the promise object ends up on the correct processor.