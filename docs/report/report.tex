...
\begin{lstlisting}[captionpos=b, caption={Migrate objects on a separate processor.}]
...
\begin{lstlisting}[captionpos=b, caption={Migrate objects with lock passing.}]
class SENDER feature
  send (a_receiver: separate RECEIVER)
      -- Invoke an asynchronous operation with
      -- an argument on `a_receiver'.
...
\begin{lstlisting}[captionpos=b, caption={Migrate objects with import.}]
class SENDER feature
  send (a_receiver: separate RECEIVER)
      -- Invoke an asynchronous operation with
      -- an argument on `a_receiver'.
...
\begin{lstlisting}[captionpos=b, caption={Breaking out of a wait condition.}]
class
  CONSUMER
...
\begin{lstlisting}[captionpos=b, caption={The deferred class CP\_IMPORT\_STRATEGY.}]
...
\begin{lstlisting}[captionpos=b, caption={An example component with import.}]
...
The Timer: InvokeLater pattern \patternref{TIL} is implemented by a single class \lstinline!CP_DEFAULT_TASK! in \dir{libary/util}.
