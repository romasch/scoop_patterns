...
This section provides a list of all patterns that we found in the initial survey.

We introduce five categories of concurrency patterns, each one with their own subsection.
The data-centric patterns all solve the general problem on how to move data from one thread to another.
Task-centric patterns all share the abstract notion of a task.
The patterns then define when and on which thread a task shall be executed.
...
The SCOOP patterns form a category simply because they're only applicable in SCOOP, and the synchronization primitives are some very basic building blocks which can be used to build concurrent data structures or higher-level patterns.

Finally, the section Miscellaneous Patterns contains several patterns which don't really fit into one of the above categories.

The structure loosely follows the one used in the book by Gamma et al \cite{book:design-patterns}, although the descriptions are a lot shorter.
...
However, for most patterns there are references to existing books and articles which can be used to look up the implementation.
