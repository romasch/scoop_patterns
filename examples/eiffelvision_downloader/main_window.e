...
feature {NONE} -- Background download events
	...
	on_terminated (is_successful: BOOLEAN)
			-- Handler for termination events generated by the download task.
		do
	...
	on_progress (progress: DOUBLE)
			-- Handler for progress change events generated by the download task.
		do
			if not is_cancelling then
				status_text.set_text ("Download progress:" + formatter.formatted (progress * 100) + "%%")
			end
		end
